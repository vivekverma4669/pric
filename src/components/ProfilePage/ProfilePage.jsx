...
//     setLoading(true);
//     try {
//       await axios.patch(`${API_URL}/api/v1/patient?patient_id=${profile.id}`, {
...
//         {isEditing && (
//           <button onClick={handleSaveClick} className="relative self-center px-14 py-5 lg:px-10 mt-11 max-w-full text-base font-bold text-center text-white bg-orange-400 border border-orange-400 rounded-[800px] w-[650px] group overflow-hidden transition-transform transform active:scale-95">
...
const ProfilePage = () => {
  ...
  const handleSaveClick = async () => {
    setLoading(true);
    try {
      await axios.put(`${API_URL}/api/v1/patient?patient_id=${profile.id}`, {
        first_name: profile.firstName,
        last_name: profile.lastName,
        dob: profile.dob,
        gender: profile.gender,
